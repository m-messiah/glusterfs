...
class Cache(object):
    ...
    def is_valid(self, key, xdata):
        return self.table.get(key).get("xdata") == xdata
...
def trace_stat2str(buf):
    ...
    return ("(gfid={0}, ino={1:d}, mode={2:o}, nlink={3:d}, uid ={4:d}, "
            "gid ={5:d}, size={6:d}, blocks={7:d}, atime={8}, mtime={9}, "
            "ctime={10})").format(gfid, buf.contents.ia_no, mode,
...
class xlator(Translator):
    ...
    def opendir_fop(self, frame, this, loc, fd, xdata):
        ...
        key = dl.get_id(frame)
        gfid = uuid2str(loc.contents.inode.contents.gfid)
        print (("GLUPY TRACE OPENDIR FOP- {0:d}: "
                "gfid={1:s}; path={2:s}; fd={3:s}")
               .format(unique, gfid, loc.contents.path, fd))
        self.gfids[key] = gfid
    ...
    def opendir_cbk(self, frame, cookie, this, op_ret, op_errno, fd,
        ...
        key = dl.get_id(frame)
        gfid = self.gfids[key]
        print(("GLUPY TRACE OPENDIR CBK- {0:d}: "
               "gfid={1:s}; op_ret={2:d}; op_errno={3:d}; fd={4:s}")
              .format(unique, gfid, op_ret, op_errno, fd))
        del self.gfids[key]
    ...
    def readdir_fop(self, frame, this, fd, size, offset, xdata):
        ...
        key = dl.get_id(frame)
        gfid = uuid2str(fd.contents.inode.contents.gfid)
        print(("GLUPY TRACE READDIR FOP- {0:d}: "
               "gfid={1:s}; fd={2:s}; size={3:d}; offset={4:d}")
              .format(unique, gfid, fd, size, offset))
        self.gfids[key] = gfid
    ...
    def readdir_cbk(self, frame, cookie, this, op_ret, op_errno, buf,
        ...
        key = dl.get_id(frame)
        gfid = self.gfids[key]
        print(("GLUPY TRACE READDIR CBK- {0:d}: "
               "gfid={1:s}; op_ret={2:d}; op_errno={3:d}")
              .format(unique, gfid, op_ret, op_errno))
        del self.gfids[key]
    ...
    def readdirp_fop(self, frame, this, fd, size, offset, dictionary):
        ...
        key = dl.get_id(frame)
        gfid = uuid2str(fd.contents.inode.contents.gfid)
        print(("GLUPY TRACE READDIRP FOP- {0:d}: "
               "gfid={1:s}; fd={2:s};  size={3:d}; offset={4:d}")
              .format(unique, gfid, fd, size, offset))
        self.gfids[key] = gfid
    ...
    def readdirp_cbk(self, frame, cookie, this, op_ret, op_errno, buf,
        ...
        key = dl.get_id(frame)
        gfid = self.gfids[key]
        print(("GLUPY TRACE READDIRP CBK- {0:d}: "
               "gfid={1:s}; op_ret={2:d}; op_errno={3:d}")
              .format(unique, gfid, op_ret, op_errno))
        del self.gfids[key]
        dl.unwind_readdirp(frame, cookie, this, op_ret, op_errno,
                           buf, xdata)
        return 0
    ...
    def mkdir_fop(self, frame, this, loc, mode, umask, xdata):
        unique = dl.get_rootunique(frame)
        gfid = uuid2str(loc.contents.inode.contents.gfid)
        print(("GLUPY TRACE MKDIR FOP- {0:d}: "
               "gfid={1:s}; path={2:s}; mode={3:d}; umask=0{4:o}")
              .format(unique, gfid, loc.contents.path, mode, umask))
        dl.wind_mkdir(frame, POINTER(xlator_t)(), loc, mode,
                      umask, xdata)
        return 0
    ...
    def mkdir_cbk(self, frame, cookie, this, op_ret, op_errno, inode, buf,
        ...
        if op_ret == 0:
            ...
            preparentstr = trace_stat2str(preparent)
            postparentstr = trace_stat2str(postparent)
            print(("GLUPY TRACE MKDIR CBK- {0:d}: "
                   "gfid={1:s}; op_ret={2:d}; *stbuf={3:s}; "
                   "*prebuf={4:s}; *postbuf={5:s} ")
                  .format(unique, gfid, op_ret,
                          statstr, preparentstr, postparentstr))
        else:
            print(("GLUPY TRACE MKDIR CBK- {0:d}: "
                   "op_ret={1:d}; op_errno={2:d}")
                  .format(unique, op_ret, op_errno))
    ...
    def rmdir_fop(self, frame, this, loc, flags, xdata):
        ...
        key = dl.get_id(frame)
        gfid = uuid2str(loc.contents.inode.contents.gfid)
        print(("GLUPY TRACE RMDIR FOP- {0:d}: "
               "gfid={1:s}; path={2:s}; flags={3:d}")
              .format(unique, gfid, loc.contents.path, flags))
        self.gfids[key] = gfid
    ...
    def rmdir_cbk(self, frame, cookie, this, op_ret, op_errno, preparent,
        ...
        if op_ret == 0:
            preparentstr = trace_stat2str(preparent)
            postparentstr = trace_stat2str(postparent)
            print(("GLUPY TRACE RMDIR CBK- {0:d}: "
                   "gfid={1:s}; op_ret={2:d}; *prebuf={3:s}; "
                   "*postbuf={4:s}")
                  .format(unique, gfid, op_ret,
    ...
    def stat_fop(self, frame, this, loc, xdata):
        ...
        key = dl.get_id(frame)
        gfid = uuid2str(loc.contents.inode.contents.gfid)
        print("GLUPY TRACE STAT FOP- {0:d}: gfid={1:s}; path={2:s}"
              .format(unique, gfid, loc.contents.path))
        self.gfids[key] = gfid
    ...
    def stat_cbk(self, frame, cookie, this, op_ret, op_errno, buf,
        ...
        if op_ret == 0:
            ...
            print(("GLUPY TRACE STAT CBK- {0:d}: gfid={1:s}; "
                   "op_ret={2:d};  *buf={3:s};")
                  .format(unique, gfid, op_ret, statstr))
        else:
            print(("GLUPY TRACE STAT CBK- {0:d}: gfid={1:s}; "
    ...
    def fstat_fop(self, frame, this, fd, xdata):
        ...
        key = dl.get_id(frame)
        gfid = uuid2str(fd.contents.inode.contents.gfid)
        print("GLUPY TRACE FSTAT FOP- {0:d}:  gfid={1:s}; fd={2:s}"
              .format(unique, gfid, fd))
        self.gfids[key] = gfid
    ...
    def getxattr_fop(self, frame, this, loc, name, xdata):
        ...
        print("GLUPY TRACE GETXATTR FOP- {0:d}: gfid={1:s}; path={2:s};"
              " name={3:s}").format(unique, gfid, loc.contents.path, name)
        self.gfids[key] = gfid
    ...
    def link_fop(self, frame, this, oldloc, newloc, xdata):
        ...
        print(("GLUPY TRACE LINK FOP-{0:d}: oldgfid={1:s}; oldpath={2:s};"
               "newgfid={3:s}; newpath={4:s}")
              .format(unique, oldgfid, oldloc.contents.path,
                      newgfid, newloc.contents.path))
